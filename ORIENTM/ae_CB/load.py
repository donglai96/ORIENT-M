...
def load(start_time = None, end_time = None, no_update = False):
    """
    Load the ae DATA from colorado's model
    ...
    filenames = [CONFIG['local_data_dir'] + 'ae_' + x + name_suffix for x in month_str_list]
    urls = [CONFIG['remote_data_dir'] + 'ae_' + x + name_suffix for x in month_str_list]
    ...
    insert_index = np.where(time_test[1:] - time_test[:-1] > 288*mingap) # 36 here is 3 hour
    add_total = 0
    print(insert_index)
